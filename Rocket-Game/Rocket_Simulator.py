...
from Tk_Animation import AnimationWindow, tkinter
import time
...
class Rocket(object):
    ''' 
    Rocket with one main engine at the tail,
    two RCS thrusters on the left and right
    The main engine provides force to move the rocket forwards, 
    the RCS(reaction control system) thrusters provide  
    around the rocket's center of mass'''

    def __init__(self, world_width, world_height, initial_x_pos=None):
        self.world_width = world_width
        self.world_height = world_height
        self.rocket_vertices = [
            (-2, 0), (-1, 1), (-1, 7), (0, 8), (1, 7), (1, 1), (2, 0)
        ]
        self.rotation_point = (0, 2.5)  # center of mass?
        self.engine_flame_vertices = [
            (-1, 0), (-1.5, -2), (-0.5, -2), (-1, -4),
            (0, -3), (1, -4), (0.5, -2), (1.5, -2), (1, 0)
        ]
        self.thruster_positions = [(-1.5, 7), (1.5, 7)]
        self.set_touchdown_position(initial_x_pos or 0.0)
    ...
    def draw_calls(self):
        ...
        points = [v.rotate_around(
            self.rotation_point, self.rotation) for v in points]
        points = [scale*v+screen_offset for v in points]
        points = [(v.x, self.world_height - v.y) for v in points]
        calls.append(call("create_polygon", points,
                          fill="blue", outline="lightgrey"))

        if self.engine_throttle:
            # rotate and position the engine flame
            points = [Vector2D((x, y*self.engine_throttle))
        ...
        if self.left_thruster_on:
            calls.append(call("create_oval", points[0][0]-3, points[0][1]-3,
                              points[0][0]+3, points[0][1]+3, outline="orange", fill="yellow"))
        if self.right_thruster_on:
            calls.append(call("create_oval", points[1][0]-3, points[1][1]-3,
                              points[1][0]+3, points[1][1]+3, outline="orange", fill="yellow"))
        return calls
...
class Launchpad(object):
    ''' Rocket launchpad where a rocket can safely launched '''
    width = 50

    def __init__(self, canvas, x):
        self.canvas = canvas
        self.cwidth = int(canvas['width'])
        self.cheight = int(canvas['height'])
        self.x = x - self.width / 2

    def draw(self):
        self.canvas.create_rectangle(
            self.x, self.cheight - 11, self.x + self.width, self.cheight - 5,
            outline='gray80', fill='gray60'
        )
...
class RocketSimulatorWindow(AnimationWindow):
    ''' The actual landing simulation game window '''

    def setup(self):
        self.cwidth = int(self.canvas['width'])
        self.cheight = int(self.canvas['height'])
        ...
        self.launchpad_destination = Launchpad(
            self.canvas, self.cwidth - self.launchpad_offset
        )
        self.framecounter = 0
    ...
    def draw(self):
        ...
        self.canvas.create_rectangle(
            1, self.cheight - 10, self.cwidth - 1, self.cheight - 1,
            outline='chocolate', fill='sienna'
        )  # the bottom ground
        ...
        self.canvas.create_text(
            150, self.cheight / 2 - 250, text=instructions,
            fill='green4', anchor=tkinter.NW
        )
        rotation_degrees = 360 - (180 * self.rocket.rotation / math.pi)
        rotation_speed_degrees = self.rocket.rotation_speed / \
            math.pi * self.frame_rate * 180
...
""".format(self.rocket.position.x, self.rocket.position.y,
        ...
        self.canvas.create_text(
            560, self.cheight / 2 - 190, text=telemetry,
            fill='green3', anchor=tkinter.NW
        )
        ...
        if self.rocket.crashed:
            self.canvas.create_text(
                self.cwidth/2, self.cheight/2, text="ROCKET LOST !!!", fill="pink")
            self.stop() 
        if self.rocket.touchdown:
            ...
            self.canvas.create_text(
                self.cwidth/2, self.cheight/2, text="ROCKET TOUCHDOWN\n"+location, fill="pink")
        # framecounter
        fps = round(self.framecounter / (time.time() - self.start_time))
        self.canvas.create_text(self.cwidth, 0, text="FPS: {0:d} ".format(
            fps), fill="blue", anchor=tkinter.NE)
    ...
    def update(self):
        ...
        if self.rocket.engine_throttle:
            ...
            engine_force = Vector2D(
                (0, .2 * self.rocket.engine_throttle)
            ).rotate(self.rocket.rotation)
    ...
    def keypress(self, char, mouseposition):
        ...
        if char.startswith('e'):
            self.rocket.engine_throttle = 1.0
        elif char.startswith('w'):
            self.rocket.engine_throttle = 2.0
        elif char == 'left':
...
if __name__ == '__main__':
    ...
    window = RocketSimulatorWindow(
        1000, 600, 'Rocket Launch and Landing Simulation'
    )
    window.mainloop()
